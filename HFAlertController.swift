...
/// - System:       仿系统弹出样式（默认）
/// - Elasticity:   弹力样式
/// - SheetSuspend: 仿系统底部弹窗悬浮样式，底部会有10pt以及安全区的悬浮
/// - Sheet:        仿系统底部弹窗样式，底部无悬浮
...
class HFAlertController: UIAlertController, HFAlertBkViewDelegate {
    ...
    @discardableResult
    open class func showAlert(type:HFAlertType? = .Default, title: String, message: String, ConfirmTitle: String = "确定", ConfirmCallBack:((_ account: String?, _ password:String?) -> Void)?, CancelCallBack: ((UIAlertAction) -> Void)? = nil) -> HFAlertController {
        ...
        case HFAlertType.OnlyConfirm:
            alertController = HFAlertController.alertControllerByOnlyConfirm(title: title, message: message,ConfirmTitle:ConfirmTitle, ConfirmCallBack: {
                ConfirmCallBack?(nil,nil)
            })
        case .AccountAction:
    ...
    /// - Returns: 一个默认的提示框
    @objc open class func alertController(title: String, message: String, preferredStyle: UIAlertController.Style = .alert, ConfirmCallBack:(() -> Void)?, CancelCallBack: ((UIAlertAction) -> Void)? = nil) -> HFAlertController {
        
        let alertController = HFAlertController(title: title, message: message, preferredStyle: preferredStyle)
        let cancelAction = UIAlertAction(title: "取消", style: UIAlertAction.Style.cancel, handler: CancelCallBack)
    ...
    ///   - ConfirmCallBack: 确定按钮回调
    /// - Returns: return value description
    @objc open class func alertControllerByOnlyConfirm(title: String, message: String, ConfirmTitle: String = "确定", ConfirmCallBack:(() -> Void)?) -> HFAlertController {
        
        let alertController = HFAlertController(title: title, message: message, preferredStyle: UIAlertController.Style.alert)
        let yesAction = UIAlertAction(title: ConfirmTitle, style: UIAlertAction.Style.default) { (_) in
            if ConfirmCallBack != nil {
                ConfirmCallBack!()
            }
        }
    ...
    fileprivate class func alertAnimation(view: UIView,animation: HFAlertAnimationOption) {
        ...
        case .SheetSuspend:
            ...
            let newFrame = CGRect(x: frame.origin.x, y: UIScreen.main.bounds.maxY - (view.frame.height) - bottomOffset, width: frame.width, height: frame.height)
...
fileprivate class HFAlertBkView: UIView {
    ...
    fileprivate class func fullBackground() -> HFAlertBkView {
        ...
        bkView.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.5)
        bkView.isResponseEvent = false
        
        bkView.exitBkView = { [weak bkView] () in
            bkView?.exit()
        }
